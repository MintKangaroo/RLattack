...
def decide_response(
    ...
    The decision depends only on what a defender could plausibly see - accumulated
    detection risk - never on the attacker's hidden intent or the objective it chose.
    """

    if not config.enabled:
        return DefenderResponse()
    if state.detection_risk < config.alert_threshold:
        return DefenderResponse()
